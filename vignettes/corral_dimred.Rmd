...
zm4eq.corralsmooth <- corral(zm4eq.countmat, smooth = TRUE)

cppmat <- corral::corral_preproc(counts(zm8eq))
ppmat <- corral_preproc(counts(zm8eq))
ppmattrim <- corral_preproc(counts(zm8eq), smooth = TRUE)

# examining which values exactly are being changed by this procedure...
mat <- ppmat
pct_trim = .01
up_qt <- 1-(pct_trim/2)
upthresh <- quantile(mat, up_qt)
downthresh <- quantile(mat, 1-up_qt)
mat[which(mat > upthresh)] <- NA
mat[which(mat < downthresh)] <- NA

boxplot(nas ~ ct,data.frame(cbind(nas = colSums(is.na(mat)), ct = as.factor(zm8eq$phenoid))))
hist(colSums(is.na(mat)))
hist(rowSums(is.na(mat)))

# Genes with values that are being trimmed for over half of the cells
rowData(zm4eq.sce)$symbol[which(rowSums(is.na(mat)) > 2000)]

# biplots comparison
zm8eq.corral <- compsvd(ppmat)
zm8eq.corralsmooth <- compsvd(ppmattrim)

biplot_corral(zm8eq.corral, text_vec = text_vec, color_vec = ctvec, xpc = 1)
biplot_corral(zm8eq.corralsmooth, text_vec = text_vec, color_vec = ctvec, xpc = 1)
```
