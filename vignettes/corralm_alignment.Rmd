...
vignette: |
  %\VignetteIndexEntry{alignment with corral}
...
First, we will prepare the data by adding to the colData the sequencing platform, and subsetting by the intersect of the genes.

```{r}
platforms <- c('10X','CELseq2','Dropseq')
for(i in seq_along(scmix_dat)) {
  colData(scmix_dat[[i]])$Methods<- rep(platforms[i], ncol(scmix_dat[[i]]))
}
...
sapply(lapply(scmix_dat,rownames), FUN = identical, rownames(scmix_dat[[1]]))
...
* **`r Biocpkg('SingleCellExperiment')` or `ExperimentList`** does not require any specific arguments. `corralm` will identify the intersect of the rows, and use these to match the matrices. The output will be the same as for a list of matrices.
