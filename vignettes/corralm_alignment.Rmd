...
1. adding to the colData the sequencing platform (`experiment_name` in `colData` for each SCE), and 
...
```{r}
scmix_sce <- corralm(scmix_sce, splitby = 'experiment_name')
```
...
scmix_matlist <- sce2matlist(sce = scmix_sce, 
                             splitby = 'experiment_name', 
                             whichmat = 'counts')

# for plotting purposes later, while we're here
platforms <- colData(scmix_sce)$experiment_name
